-- ============================================
-- إصلاحات الأمان لقاعدة بيانات Wasla Academy (نسخة آمنة)
-- التاريخ: 2026-04-12
-- ملاحظة: هذه النسخة تتجنب تعديل الدوال غير الموجودة
-- ============================================
...
-- ============================================
-- 4. إضافة indexes للأداء والأمان
-- ============================================
...
-- Index لتسريع البحث في certificates
CREATE INDEX IF NOT EXISTS idx_certificates_student 
ON certificates(student_id);

-- Index لتسريع البحث في courses
CREATE INDEX IF NOT EXISTS idx_courses_provider_status 
ON courses(provider_id, status);

-- ============================================
-- 5. إضافة constraints إضافية للأمان
-- ============================================
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'enrollments_completion_percentage_check'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'courses_price_check'
  ) THEN
    ALTER TABLE courses 
...
-- ============================================
-- 6. إنشاء جدول للتدقيق (Audit Log)
-- ============================================
...
-- ============================================
-- 7. تحديث سياسات الجداول الحساسة
-- ============================================
...
-- ============================================
-- 8. إضافة دالة للتحقق من صلاحيات المستخدم
-- ============================================
...
BEGIN
  ...
  RAISE NOTICE '  4. إضافة indexes للأداء (8 indexes)';
  RAISE NOTICE '  5. إضافة constraints للأمان (4 constraints)';
  RAISE NOTICE '  6. إنشاء جدول التدقيق (audit_log)';
  RAISE NOTICE '  7. تحديث سياسات الجداول الحساسة';
  RAISE NOTICE '  8. إضافة دالة التحقق من الصلاحيات';
  RAISE NOTICE '';
  RAISE NOTICE '⚠️ تذكير: قم بتفعيل حماية كلمات المرور المسربة من Dashboard';
  RAISE NOTICE '⚠️ ملاحظة: لم يتم تعديل الدوال الموجودة لتجنب الأخطاء';
END $$;
